...
export class GqlAuthGuard implements CanActivate ,OnModuleInit{
   ...
   canActivate(context: ExecutionContext): boolean | Promise<boolean> | Observable<boolean> {
      ...
      return this.authService.authenticate({token}).pipe(map((res)=>{ this.getRequest(context).user = res
        this.logger.log(res);return true}),catchError((err)=> {return of(false)}))
  }
